...
export const AssetsCrypto: React.FC = () => {
  ...
  const assetFromLocalStorage = JSON.parse(localStorage.getItem('assets') || '[]');
  ...
  const onClickAddNewAssetToLocalStorage = (asset: any) => {
    const assets = JSON.parse(localStorage.getItem('assets') || '[]');
    if (!assets.find((a: any) => a.id === asset.id)) {
      assets.push(asset);
  ...
  useEffect(() => {
    ...
    assets.slice(0, 2).forEach(async (asset) => {
      ...
      setHistoryPrices((prevPrices) => ({ ...prevPrices, [asset.id]: prices }));
    });

    assetFromLocalStorage.forEach(async (asset: any) => {
      const prices = await getHistoricalData({
        id: asset.id,
        vs_currency: 'usd',
        days: 180,
  ...
  }, [status, dispatch, assets, handleQueryChangeDebounced, assetFromLocalStorage]);
    ...
    <div>
      ...
      {status === 'failed' ? (
        ...
        <div className="grid grid-cols-1 gap-10 mt-10">
              ...
              <div key={asset.id}>
                ...
                <div className="bg-primary/70 rounded-lg py-7 px-5 mt-2 relative shadow-xl">
                  ...
                  <div className="absolute -right-1 top-0 w-1/2 h-full">
                    <ResponsiveContainer>
                      <AreaChart data={historyPrices[asset.id]?.formatPricesDataAreaChart}>
                        <defs>
                          <linearGradient
          ...
          {assetFromLocalStorage.map((asset: any) => {
            return (
              <div key={asset.id}>
                <p className="text-primary font-bold text-xl">{asset.name}</p>
                <div className="bg-primary/70 rounded-lg py-7 px-5 mt-2 relative shadow-xl">
